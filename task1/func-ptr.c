...
{
	Process *part_1 = (Process*)this;
	Process *part_2 = (Process*)that;
	
	// Sorted based on priority in descending order 
	if (part_1->priority > part_2->priority) {
		return -1; 
	}
	else if(part_1->priority < part_2->priority) {
		return 1;
	}
	else{
		// If priority values are equal, 
		// they are sorted based on arrival time based on ascending order 
		if (part_1->arrival_time > part_2->arrival_time) {
			return 1;
		}
		else{
			return -1;
